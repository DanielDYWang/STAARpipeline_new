                                                              ...
                                                              FALSE), Annotation_name = NULL, silent = FALSE, MAC_super_rare = MAC_super_rare, cMAC_super_rare = cMAC_super_rare) 
{
  ...
  if (!is.null(results)) {
    ...
    if (dim(results)[1] == 1) {
      if (results[3] != "disruptive_missense") {
        ...
        colnames(results)[(dim(results)[2] - 5):dim(results)[2]] <- c("SKAT(1,25)-Disruptive", 
                                                                      "SKAT(1,1)-Disruptive", "Burden(1,25)-Disruptive", 
                                                                      "Burden(1,1)-Disruptive", "ACAT-V(1,25)-Disruptive", 
                                                                      "ACAT-V(1,1)-Disruptive")
    ...
    if (!is.null(results)) {
      if (dim(results)[1] == 2) {
        ...
        names(results_m)[(length(results_m) - 5):length(results_m)] <- c("SKAT(1,25)-Disruptive", 
                                                                         "SKAT(1,1)-Disruptive", "Burden(1,25)-Disruptive", 
                                                                         "Burden(1,1)-Disruptive", "ACAT-V(1,25)-Disruptive", 
                                                                         "ACAT-V(1,1)-Disruptive")
        results_m[(length(results_m) - 5):length(results_m)] <- results[2, 
                                                                        c("SKAT(1,25)", "SKAT(1,1)", "Burden(1,25)", 
                                                                          "Burden(1,1)", "ACAT-V(1,25)", "ACAT-V(1,1)")]
        apc_num <- (length(results_m) - 19)/6
        p_seq <- c(1:apc_num, 1:apc_num + (apc_num + 
                                             1), 1:apc_num + 2 * (apc_num + 1), 1:apc_num + 
                     3 * (apc_num + 1), 1:apc_num + 4 * (apc_num + 
                                                           1), 1:apc_num + 5 * (apc_num + 1), (6 * apc_num + 
                                                                                                 9):(6 * apc_num + 14))
        save(results_m, file="/restricted/projectnb/adsp-charge/danielwang/36K/STAAR_report_results/revised_results/36k/coding_m1/results_m.Rdata")
        num <- lapply(results_m[86:length(results_m)], is.numeric)
        results_m1 <- results_m[86:length(results_m)][which(num==T)]
      
        
        results_m["STAAR-O"] <- CCT(as.numeric(results_m1))
        # results_m["STAAR-S(1,25)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num, 
        #                                                                               6 * apc_num + 9)]))
        # results_m["STAAR-S(1,1)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
        #                                                                                (apc_num + 1), 6 * apc_num + 10)]))
        # results_m["STAAR-B(1,25)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
        #                                                                                 2 * (apc_num + 1), 6 * apc_num + 11)]))
        # results_m["STAAR-B(1,1)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
        #                                                                                3 * (apc_num + 1), 6 * apc_num + 12)]))
        # results_m["STAAR-A(1,25)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
        #                                                                                 4 * (apc_num + 1), 6 * apc_num + 13)]))
        # results_m["STAAR-A(1,1)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
        #                                                                                5 * (apc_num + 1), 6 * apc_num + 14)]))
        results_ds <- c()
  ...
  results_coding <- list(plof = results_plof, plof_ds = results_plof_ds, 
                         missense = results, disruptive_missense = results_ds, 
...
#                                                            1), 1:apc_num + 5 * (apc_num + 1), (6 * apc_num + 
#                                                                                                  9):(6 * apc_num + 14))
#         save(results_m,file = "results_m.Rdata")
#         results_m["STAAR-O"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num,1:apc_num + 
#                                                                                   (apc_num + 1),1:apc_num + 
#                                                                                   2 * (apc_num + 1),1:apc_num + 
#                                                                                   3 * (apc_num + 1),6 * apc_num + 9,6 * apc_num + 10,6 * apc_num + 11, 6 * apc_num + 12)])) #
#         results_m["STAAR-S(1,25)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num, 
#                                                                                       6 * apc_num + 9)]))
#         results_m["STAAR-S(1,1)"] <- CCT(as.numeric(results_m[6:length(results_m)][c(1:apc_num + 
#                                                                                        (apc_num + 1), 6 * apc_num + 10)]))
